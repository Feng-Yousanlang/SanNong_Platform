...
CREATE TABLE `user_address` (
                                ...
                                `address_name` varchar(200) DEFAULT NULL COMMENT '用户详细地址',
...
CREATE TABLE `repayment_plan` (
                                  ...
                                  `status` varchar(10) NOT NULL COMMENT '状态（未还/已还/逾期）',
...
CREATE TABLE `repayment_record` (
                                    ...
                                    `status` varchar(10) NOT NULL COMMENT '状态（已还/部分还款）',
                                    `created_at` timestamp NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
                                    PRIMARY KEY (`recordId`),
...
-- 专家档案由 docs/seed-data.sql 导入（含王专家 / 李教授，对应 expert01 等测试账号）
